...
label: { 
	...
	/*========printing dates=========*/
		...
		printchar(CH);
		attroff(A_BOLD);
		mvprintw(row-5,0,"(*) press N for next year",row,col);
		mvprintw(row-4,0,"(*) use LEFT RIGHT UP DOWN arrow",row,col);
		mvprintw(row-3,0,"(*) press P to save calender",row,col);
		mvprintw(row-2,0,"(*) press I for new month and new year",row,col);
		mvprintw(row-1,0,"(*) press ESC to exit",row,col);
		keycode = getkey();
